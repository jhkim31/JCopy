...
const gRPCClient = new StorageProto.jcopy.Storage(`${config.grpc.StorageService.host}:${config.grpc.StorageService.port}`, grpc.credentials.createInsecure());
...
gRPCServer.addService(RoomProto.jcopy.Room.service, {
    CreateRoom: async (CreateRoomRequest, responseCallBack) => {
        ...
        async function CreateTextCB(error, CreateTextResponse) {
            if (error) {
                logger.error(`  [2-102-51] gRPC CreateText Error RPC_ID : ${CreateTextRequest.id}: ${JSON.stringify(error, null, 4)}`);
